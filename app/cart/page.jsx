...
function page() {
  ...
  const totalAmountLength = getTotalAmount()
...
<section>
  <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8 ">
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mt-8">
        {cart.length>0 ?   <ul className="space-y-4">
            {cart?.map((item)=>(
  ...
  <div>
    ...
    <button onClick={()=> deleteCartItemFromList(item?.id)} className="text-gray-400 transition hover:text-red-600 ">
      ...
      >
        ...
        </ul>: <Lottie style={{height: 400}} animationData={emptyCart}/>
			}
        ...
        <div className="mt-8 flex justify-end border-t border-gray-100 pt-8">
          <div className="w-screen max-w-lg space-y-4">
            ...
            {totalAmountLength>0 ?<button
             
             onClick={()=> router.push(`/checkout?amount=${getTotalAmount()}`)}
               className="block rounded bg-primary px-5 py-3 text-sm text-gray-100 transition hover:bg-teal-600"
             >
               Checkout
             </button>:""}
        ...
        <h2 className='text-gray-500 text-[12px]'>Note: All Items will be Send  Via Email</h2>
      </div>
    </div>
  </div>
</section>
